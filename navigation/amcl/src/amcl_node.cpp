...
class AmclNode
{
  ...
  private:
    ...
    void laserReceived_new(const sensor_msgs::LaserScan& laser_scan); // Brian added.
    ...
    ros::Subscriber laser_scan_sub_new;
        ...
        first_reconfigure_call_(true)
{
  ...
  ROS_WARN("laser_scan_sub_ SUBSCRIBE TO LASER_SCAN MSGS");
  // laser_scan_sub_new = nh_.subscribe(scan_topic_,100,  &AmclNode::laserReceived_new, this);// Brian added.
  // ROS_WARN("laser_scan_sub_new SUBSCRIBE TO LASER_SCAN MSGS");
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
  ...
  if(laser_model_type_ == LASER_MODEL_BEAM)
    ...
    ROS_INFO("Initializing likelihood field model; this can take some time on large maps..."); //TODO: WHAT IS THE LASER_MODEL_TYPE
...
{
  // ROS_WARN("This is checkLaserReceived loop"); //Brian added
...
{ 
  // ROS_INFO("This is laserscan receiving loop"); //Brian added, 0418; it seems like it didn't go through this loop!?
  std::string laser_scan_frame_id = stripSlash(laser_scan->header.frame_id);
  // std::cout<<"void Laser_received, laser_scan_frame_id: "<< laser_scan_frame_id <<std::endl;
...
// Brian added.
void AmclNode::laserReceived_new(const sensor_msgs::LaserScan& laser_scan){ 
  // ROS_WARN("This is laser_scan receive loop with Ptrlaserscan loop"); //Brian added
  std::string laser_scan_frame_id = stripSlash(laser_scan.header.frame_id);
  std::cout<<"void Laser_received, laser_scan_frame_id: "<< laser_scan_frame_id <<std::endl;
  last_laser_received_ts_ = ros::Time::now();
  if( map_ == NULL ) {
